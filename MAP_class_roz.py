...
class MAP_roz:
    ...
    def populate_data(self, meas, nb_max, nbr_cell_hor, nbr_cell_vert, node_horizontal_user, node_vertical_user,
        ...
        if all(deg == 0 for deg in meas.transects[checked_transect_idx[0]].sensors.heading_deg.internal.data):
            ...
            left_geometry, right_geometry, dmg = MAP_roz.collect_data(meas, navigation_reference, checked_transect_idx,
                                                                  interp_option)
            self.orig_start_edge = orig_start_edge

            # Compute coefficients of the average cross-section
            alpha, beta, direction_meas, q_ens = MAP_roz.compute_coef(x_raw_coordinates, y_raw_coordinates,
                                                                  w_vel_x, w_vel_y, q_cells, track_section)

            # Project raw coordinates on average cross-section
            x_projected, y_projected, acs_distance_raw = MAP_roz.project_transect(alpha, beta, x_raw_coordinates,
                                                                              y_raw_coordinates)

            # Compare bathymetry and translate transects on average cross-section if needed
            acs_distance = MAP_roz.translated_transects(acs_distance_raw, depth_data)
            ...
            borders_ens_raw, nodes_depth_raw = MAP_roz.compute_node_size(cell_depth, acs_distance,
            ...
            transects_node_depth, transects_nodes = MAP_roz.compute_nodes_velocity(navigation_reference,
    ...
    def compute_node_size(cell_depth, acs_distance, depth_data, node_horizontal_user,
        ...
        print("Profondeur premire noeud = ", nodes_depth[0])
    ...
    def plot_profile(self, borders_ens_raw, nodes_depth_raw, left_mid_cells_x, right_mid_cells_x,
        ...
        cb.ax.set_ylabel(canvas.tr('Vitesse Primaire Rozovskii / Méthode Classique (m/s)'))
        ...
        plt.title("Colormap de la projection de Rozovskii avec la méthode classique")
        
        plt.savefig("Images_ppt/Classic_ROZ.png",bbox_inches='tight',pad_inches=0.1)
